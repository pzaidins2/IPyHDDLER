...
def open__a__stack__and__recurse( state, rigid ):
	for n in rigid.count:
		for n1 in rigid.count:
			if any( all( [ ( n, ) in state.stacks__avail, ( n, n1, ) in rigid.next__count, ] ) for ( n, n1, ) in itertools.product( rigid.count, rigid.count, ) ):
...
def repair__an__order( state, rigid ):
	for avail in rigid.count:
		for avail__prime in rigid.count:
			for o in rigid.order:
				if all( [ ( o, ) in rigid.goal__shipped, not( ( o, ) in state.shipped ), not( ( o, ) in state.started ), all( any( [ not( ( o, p, ) in rigid.includes ), ( p, ) in state.made, ] ) for ( p, ) in itertools.product( rigid.product, ) ), ( avail__prime, avail, ) in rigid.next__count, ] ):
